...
bool pausing = false; // Indicate pausing status from admin console
...
int primary_index = 1; // Define initial primary node's index
int log_count = 0;
...
void StorageServer::ReinitializeTablet() {
    this->tablet.data = map<string, map<string, vector<char>>>();
}
...
/* Parse Config when replication not implemented (no longer used) */
...
/* Parse Config when replication implemented */
...
void StorageServer::Worker(void* param) {
    ...
    bool write_log; // Indicate whether need to write to log (PUT, GET and DELETE need to write to log)
    ...
    while(!shutting_down) {
        ...
        string print_message = readin_string;
        if (readin_string.size() > 20) {
            print_message = readin_string.substr(0,20);
        }
        PrintLog(vflag_, "[%d] Read in message: %s",comm_fd, print_message.c_str());
        ...
        /* if not alive and command not from admin, response error msg */
        if (!is_alive && !(command == "PAUS" || command == "CNTE")) {
            string err_msg = "-ERR Server Paused\r\n";
            DoWrite(comm_fd, err_msg.c_str(), err_msg.length());
            continue;
        }
        
        if (command == "FORW") {
            /* received message forwarded from primary node */
...
bool StorageServer::ProcessOperations(int comm_fd, string command, string message, string readin_string, int type) {
    ...
    if (command == "PUTT") {
        ...
        /* restart/continue operation */
        is_alive = true;
        log_count = 0;
        this->ReinitializeTablet();
        ReadLog();
        write_log = false;
...
/* Handler for forwarding message */
void StorageServer::ForwardWorker() {
    ...
    while(!shutting_down && is_alive && (server_index == primary_index)) {
...
/* Send heartbeats (server status) to master node */
...
/* Create log file when server starts or clear log file when log counts reached limit */
void StorageServer::CreateLog() {
    string path_log = "./log/" + this->server_string_id + "_log.txt";
    ofstream file_log(path_log);
}

/* Create checkpoint file when server starts */
...
/* Put write operations into log file */
...
/* Put current tablet into checkpoint file */
...
/* Clear log file, can be substitue by CreateLog function */
...
/* Read config file to get servers ip */
...
int StorageServer::Run() {
    ...
    /* Primary forward message in a single thread */
    if (primary_index == server_index && !forward_worker_alive) {
        thread forward_worker_thread([&] {ForwardWorker(); });
        forward_worker_thread.detach();
    }
...
/* Get primary node and log when recovering */
...
/* Reset index when assignming new primary */
...
/* Check if log exist during recovering */
